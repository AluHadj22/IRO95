...
"""
        ...
        try:
            ...
            for course in courses:
                ...
                if speakers:
                    ...
                    speakers_text = f" | Преподаватель(и): {', '.join(speaker_names)}"
                ...
                if course.current_participants >= course.max_participants:
                    ...
                    places_status = f" (свободно {course.max_participants - course.current_participants} мест)"
                
                # Форматируем более структурированно
                result.append(
                    f"Название: {course.title}\n"
                    f"Описание: {description}\n"
                    f"Формат: {course.format_type or 'онлайн'}\n"
                    f"Даты: {start_date} - {end_date}\n"
                    f"Участники: {course.current_participants}/{course.max_participants}{places_status}\n"
                    f"{speakers_text}\n"
                )
    ...
    def get_teachers_context(self, limit: int = 5) -> str:
        ...
        try:
            ...
            for name, data in list(speaker_dict.items())[:limit]:
                courses_text = ", ".join(data['courses'][:3]) if data['courses'] else "нет активных курсов"
    ...
    def get_full_context(self) -> str:
        ...
        try:
            ...
            if courses and "Активных курсов нет" not in courses and "временно недоступна" not in courses:
                context_parts.append("\n=== ТЕКУЩИЕ КУРСЫ (ДОСТУПНЫ ДЛЯ ЗАПИСИ) ===")
                context_parts.append(courses)
            elif courses and "Активных курсов нет" in courses:
            ...
            if teachers and "нет назначенных преподавателей" not in teachers and "временно недоступна" not in teachers:
                context_parts.append("\n=== ПРЕПОДАВАТЕЛИ ===")
