...
def delete_category(category_id: int, db: Session = Depends(get_db),
    ...
    # Для всех курсов в этой категории убираем category_id
...
def export_course_registrations(course_id: int, db: Session = Depends(get_db),
                                current_user: models.User = Depends(auth.get_current_admin)):
    course = db.query(models.Course).filter(models.Course.id == course_id).first()
    if not course:
        raise HTTPException(status_code=404, detail="Course not found")
    ...
    data = [{"full_name": r.User.full_name, "email": r.User.email, "phone": r.User.phone or "",
             "position": r.User.position or "", "organization": r.User.organization or "",
             "is_paid": "Да" if r.CourseRegistration.is_paid else "Нет",
             "registered_at": r.CourseRegistration.registered_at} for r in registrations]
    
    buffer = generate_registrations_excel(data)
    return StreamingResponse(buffer,
                            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            headers={"Content-Disposition": f"attachment; filename=course_{course_id}_registrations.xlsx"})
